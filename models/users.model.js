...
var saveNewUser = function(params,callback){
    ...
    Admin.findOne(query,exclude).exec(function(error,adminResult){
        if(error){
            ...
            Admin.findOne({email:params.email},function(error,newExpert){
                if(error){
                    return callback(error);
                }else if(!newExpert){
...
var findAllExperts = function(params,callback){
    ...
    Admin.findOne(query,exclude).exec(function(error,adminResult){
        if(error){
            ...
            Admin.find({role:'expertUser'},{__v:0,password:0},function(error,experts){
                if(error){
                    return callback(error);
                }
...
var getExpertById = function(params,callback){
    ...
    Admin.findOne(query,exclude).exec(function(error,adminResult){
        if(error){
            ...
            Admin.findOne({_id:params.expertId,role:'expertUser'},{__v:0,password:0},function(error,expertData){
                if(error){
                    return callback(error);
                }else if(expertData){
...
var getMydetails = function(params,callback){
    ...
    Admin.findOne({_id:params.userId},{__v:0,password:0}).exec(function(error,userResult){
        if(error){
            return callback(error);
        }
...
var updateImage = function(params,callback){
    ...
    Admin.findOneAndUpdate({_id:params.userId},{$set:{profileImage:params.profileImage}},{new: true}).exec(function(err, user){
        if (err) {
            return callback(err);
        }
...
var updateUserProfile = function(params,callback){
    ...
    Admin.findByIdAndUpdate(params.userId,{$set:userResult},{new:true},function(error,updatedResult){
        if(error){
            return callback(error);
        }
